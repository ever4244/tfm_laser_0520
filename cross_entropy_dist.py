...
class CrossEntropyCriterion_dist(FairseqCriterion):
    ...
    def forward_dist(self, model, sample):
        ...
        tgt_lengths, tgt_sort_order = tgt_lengths.sort(descending=True)
        tgt_tokens = tgt_tokens.index_select(0, tgt_sort_order)
        net_output = model(**sample['net_input'])
        
        #print ('net_output = {}'.format(net_output))
        src_embeddings=net_output[-1]['sentemb']
        tgt_embeddings_sorted=model.encoder(tgt_tokens, tgt_lengths)['sentemb']

        arg_tgt_sort_order=tgt_sort_order.argsort(-1,descending=False)
        #arg_tgt_sort_order=tgt_sort_order.argsort(tgt_sort_order)
        #print('tgt_sort_order = {} \n arg_tgt_sort_order = {}'.format(tgt_sort_order,arg_tgt_sort_order))
        tgt_embeddings=tgt_embeddings_sorted.index_select(0, arg_tgt_sort_order)
        tgt_lengths=tgt_lengths.index_select(0, arg_tgt_sort_order)
